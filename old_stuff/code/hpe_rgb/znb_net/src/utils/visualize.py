...
def blend_scoremap_img(scoremap, img, alpha=0.5):
    scoremap_show = cv2.cvtColor(scoremap, cv2.COLOR_GRAY2RGB)
    scoremap_show = cv2.normalize(scoremap_show, None, alpha = 0, beta = 255, norm_type = cv2.NORM_MINMAX, dtype = cv2.CV_8UC3)
    return (alpha*scoremap_show + (1-alpha)*img).astype('uint32')
