...
class EgoDexter_Bbox_Dataset(torch.utils.data.Dataset):
    def __init__(self, cfg, split):
        ...
        split_set = Path(ROOT)/(cfg['{}_set'.format(split)]).replace('img', 'bbox_pad{}'.format(cfg['pad']))
        self.bbox_gt = np.loadtxt(split_set)
