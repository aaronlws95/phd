...
class Base_Model:
    ...
    def __init__(self, cfg):
        ...
        self.tbx_logger         = TBX_Logger(self.exp_dir/'log')
        self.load_epoch         = int(cfg['load_epoch'])
        ...
        self.save_ckpt_dir  = self.exp_dir/'ckpt'
        if not self.save_ckpt_dir.is_dir():
            self.save_ckpt_dir.mkdir(parents=True, exist_ok=True)

        if self.mode not in ['train', 'test']:
            raise ValueError('Mode must be train or test, not\"{}\"'.format(self.mode))
    ...
    def _load_ckpt(self, load_dir):
        if len(self.device) > 1:
            ...
            map_loc = 'cuda:' + str(self.device[0])
