...
# jnt_in_prev_layer=[cur_finger*4+1+1]
version = 'dtip_s0_finger%d_smalljiter_ker%d_lr%f'%(cur_finger,num_kern[0],lr)
...
def load_data(save_dir,dataset,cur_jnt_idx,cur_finger):
    normuvd = numpy.load("%s/hier/result/%s_normuvd_tmp_vass_palm_s0_rot_scale_ker32_lr0.000100.npy"%(save_dir,dataset))
    jnt_uvd_in_prev_layer = numpy.load("%s/hier/result/%s_normuvd_pip_s0_finger%d_smalljiter_ker48_lr0.000100.npy"%(save_dir,dataset,cur_finger))
...
def train():
    ...
    model = multi_resolution.get_multi_reso_for_finger(img_rows=48,img_cols=48,
                                             num_kern=num_kern,num_f1=1024,num_f2=1024,cnn_out_dim=6)
    ...
    # model.save_weights("%s/hier/weight_%s"%(save_dir,version))
    train_x0, train_y,train_pred_palm,train_jnt_uvd_in_prev_layer= load_data(save_dir,dataset='train',cur_jnt_idx=cur_jnt_idx,cur_finger=cur_finger)
    test_x0,test_target,test_pred_palm,test_jnt_uvd_in_prev_layer = load_data(save_dir,dataset='test',cur_jnt_idx=cur_jnt_idx,cur_finger=cur_finger)

    trainer.fit_dtip_stage_0(batch_size=batch_size,n_epochs=1000,model=model,
          ...
          history_filepath='%s/hier/history_%s'%(save_dir,version),if_aug=True,aug_rot=15,aug_trans=0.03)
...
def predict():
    ...
    cur_finger=2
    cur_jnt_idx=[cur_finger*4+2+1,cur_finger*4+3+1]
    version='dtip_s0_finger%d_smalljiter_ker48_lr0.000100'%cur_finger
    tmp = numpy.load("%s/hier/history_%s.npy"%(save_dir,version))
    test_loss=tmp[1]
    train_loss=tmp[0]
    ...
    batch_size=128
    for dataset in ['test']:
        train_img0,train_target,train_pred_palm,train_jnt_uvd_in_prev_layer = load_data(save_dir,dataset=dataset,
                                                                                        cur_jnt_idx=cur_jnt_idx,cur_finger=cur_finger)
