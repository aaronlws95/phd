...
def evalaute_mega_online():
    ...
    for i in range(0,uvd.shape[0],1):
        print(i,file_name[i])
        ...
        for offset in all_offset:
            ...
            inSpheres[:,:,1]+=offset*perpend[0]
            inSpheres[:,:,2]+=offset*perpend[1]
            inSpheres[:,:,3]+=offset*perpend[2]
        ...
        print('offset, min cost',xopt,fopt,'ori cost',all_cost[0])
        title='%d,%s'%(i,file_name[i])
        inSpheres=sphere.copy()
        inSpheres[:,:,1]+=xopt*perpend[0]
        inSpheres[:,:,2]+=xopt*perpend[1]
        inSpheres[:,:,3]+=xopt*perpend[2]
                # cur_xyz+=xopt

        selectShpere=inSpheres[jnt_idx]
        uvd_point_label, partMap_update,ColorMap_update = LabelUtils.getPart_mega_21jnt_tmptest(setname=setname,DepthImg=depth,
        ...
        uvd_point_label, partMap,ColorMap_ori = LabelUtils.getPart_mega_21jnt_tmptest(setname=setname,DepthImg=depth,
                                     Sphere=selectShpere.reshape(len(jnt_idx)*numInSphere,5),numInSphere=numInSphere,
                                     numJnt=len(jnt_idx),pointCloudMargin=pointCloudMargin)
        ...
        img =Image.fromarray(partMap_update,mode='I')
        img.save("%s_update/%s.png"%(lable_dir,name))

        img =Image.fromarray(ColorMap_update, mode='RGB')
        img.thumbnail((640,480))
        img.save("%s_color_update/%s.jpg"%(lable_dir,name),"JPEG", quality=80)
        numpy.save('%s_xyz_update/%s.npy'%(lable_dir,name),[xopt*perpend])

        img =Image.fromarray(ColorMap_ori, mode='RGB')
        img.thumbnail((640,480))
        img.save("%s_color_ori/%s.jpg"%(lable_dir,name),"JPEG", quality=80)
