...
def norm_hand(save_path,dataset_dir,jointlocations):
    ...
    for i in xrange(0,xyz_jnt_gt.shape[0],1):
        ...
        depth = Image.open('%s%s%s' % (dataset_dir,'megahand/',file_name[i]))
...
if __name__ == '__main__':
    ...
    # jointlocations = 'training_crossobject'

    jointlocations = 'training_crossperson'
    save_path = '../../data/challenge/FristView_Cross_%s.h5' % jointlocations

    norm_hand(save_path=save_path,
    dataset_dir = '/media/Data/shanxin/', jointlocations = jointlocations)
