...
def rot_conv_img(r0,r1,r2,pred_uvd,gr_uvd ,rotation,batch_size):
    ...
    conv00_kern_size=constants.conv00_kern_size
    conv00_pool_size=constants.conv00_pool_size
    conv10_kern_size=constants.conv10_kern_size
    conv10_pool_size=constants.conv10_pool_size
    conv20_kern_size=constants.conv20_kern_size
    conv20_pool_size=constants.conv20_pool_size
    # conv00_kern_size=5
    # conv00_pool_size=4
    # conv10_kern_size=3
    # conv10_pool_size=2
...
def rot_loc_prev_cur_uvd(prev_pred_uvd, gr_uvd,rotation):
    prev_pred_uvd_derot = numpy.empty_like(prev_pred_uvd)
    prev_pred_uvd_derot[:,:,2]=prev_pred_uvd[:,:,2]
    ...
    for i in xrange(0,gr_uvd.shape[0],1):
        ...
        for j in xrange(prev_pred_uvd.shape[1]):
            prev_pred_uvd_derot[i,j,0:2] = numpy.dot(M,numpy.array([prev_pred_uvd[i,j,0]*72+12,prev_pred_uvd[i,j,1]*72+12,1]))/96
        for j in xrange(gr_uvd.shape[1]):
            gr_uvd_derot[i,j,0:2] = numpy.dot(M,numpy.array([gr_uvd[i,j,0]*72+12,gr_uvd[i,j,1]*72+12,1]))/96

    return prev_pred_uvd_derot,gr_uvd_derot

def uvd_derot(r0,r1,r2,
              prev_pred_uvd,gr_uvd,
              prev_jnt_idx,
              batch_size):
    rotation = get_rot(prev_pred_uvd,0,3)
    prev_pred_uvd_derot, gr_uvd_derot= rot_loc_prev_cur_uvd(prev_pred_uvd=prev_pred_uvd, gr_uvd=gr_uvd,rotation=rotation)

    pred_patch_center,gr_patch_center=rot_conv_img(r0,r1,r2,prev_pred_uvd[:,1:6,:],gr_uvd[:,prev_jnt_idx[1:6],:],rotation,batch_size)
    # for i in numpy.random.randint(0,r0.shape[0],10):
    #     fig = plt.figure()
    #     ax = fig.add_subplot(131)
    #     ax.imshow(r0[i,2,:,:],'gray')
    ...
    #     ax.imshow(r2[i,2,:,:],'gray')
    #     plt.show()
    return rotation,prev_pred_uvd_derot, gr_uvd_derot,pred_patch_center,gr_patch_center
