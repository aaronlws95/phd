...
for i in xrange(0,21,1):
    ...
    holi_derot_recur_xyz[:,i,:]=scipy.io.loadmat('%sdata/HDJIF_cmp_prior/NYU_hybrid_v2/jnt%d_xyz.mat'% (dataset_path_prefix,i))['jnt']
err_hybrid = numpy.sqrt(numpy.sum((holi_derot_recur_xyz - xyz_true)**2,axis=-1))

print ' 21jnt hybrid_v2 error ',numpy.mean(numpy.mean(err_hybrid ))
print ' 11jnt hybrid_v2 error ',numpy.mean(numpy.mean( err_hybrid[:,part_jnt_idx_for_us]))
...
plt.grid('on')
plt.show()
# plt.savefig('C:/Proj/Proj_Hier_Hand_eccv2016_07042016/Proj_CNN_Hier/data/HDJIF_cmp_prior/iros2016_fig/nyucmppropjnt.eps',format='eps', dpi=300)
