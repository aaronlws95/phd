...
for i in xrange(0,21,1):
    ...
    hier_derot_recur_xyz[:,i,:]=scipy.io.loadmat('%sdata/HDJIF_cmp_prior/NYU_hier_derot_recur_v2/jnt%d_xyz.mat'% (dataset_path_prefix,i))['jnt']
...
print '21jnts hier_derot_recur_v2 error ',numpy.mean(numpy.mean( numpy.sqrt(numpy.sum((hier_derot_recur_xyz - xyz_true)**2,axis=-1))))
print '11jnts hier_derot_recur_v2 error ',numpy.mean(numpy.mean( numpy.sqrt(numpy.sum((hier_derot_recur_xyz[:,part_jnt_idx_for_us,:] - xyz_true[:,part_jnt_idx_for_us,:])**2,axis=-1))))

err_hier_derot_recur_v2 = numpy.max(numpy.sqrt(numpy.sum((hier_derot_recur_xyz[:,part_jnt_idx_for_us,:] - xyz_true[:,part_jnt_idx_for_us,:])**2,axis=-1))*1000,axis=-1)
