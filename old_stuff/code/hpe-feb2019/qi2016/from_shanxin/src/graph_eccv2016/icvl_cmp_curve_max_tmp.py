...
# hier_derot_recur_xyz =scipy.io.loadmat('%sdata/HDJIF_cmp_prior/ICVL_hybrid_v2/ICVL_21_16.mat'% (dataset_path_prefix))['CNNResultDist2GT']
# err_hybrid_v2 = numpy.max(hier_derot_recur_xyz[:,idx],axis=-1)
# print '21jnts hybrid_v2 error ',numpy.mean(numpy.mean(hier_derot_recur_xyz))
# print '15jnts hybrid_v2 error ',numpy.mean(numpy.mean(hier_derot_recur_xyz[:,idx]))

hier_derot_recur_xyz =scipy.io.loadmat('%sdata/HDJIF_cmp_prior/ICVL_hybrid_v2/ICVL_21_16.mat'% (dataset_path_prefix))['CNNResultDist2GT16']
err_hybrid_v2 = numpy.max(hier_derot_recur_xyz,axis=-1)

print '16jnts hybrid_v2 error ',numpy.mean(numpy.mean(hier_derot_recur_xyz))
...
for t in xrange(len(thresholds)):
    ...
    hier_derot_recur_suc_rate[t] = 100.0 * loc_bl_t[0].shape[0] / err_hybrid_v2.shape[0]
