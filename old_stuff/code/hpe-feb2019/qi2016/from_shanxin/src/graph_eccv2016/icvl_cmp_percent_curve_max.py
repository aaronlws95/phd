...
for i in xrange(0,21,1):
    # hier_derot_recur_xyz[:,i,:]=scipy.io.loadmat('%sdata/HDJIF_cmp_prior/ICVL_hier_derot_recur_v2_1050/jnt%d_xyz.mat'% (dataset_path_prefix,i))['jnt']
    hier_derot_recur_xyz[:,i,:]=scipy.io.loadmat('%sdata/HDJIF_cmp_prior/ICVL_hier_derot_recur_v2/jnt%d_xyz.mat'% (dataset_path_prefix,i))['jnt']


thresholds=range(0,81,4)
hier_derot_recur_suc_rate = numpy.zeros((len(thresholds)),dtype=theano.config.floatX)
mean_err= numpy.mean((numpy.sqrt(numpy.sum((hier_derot_recur_xyz- xyz_true)**2,axis=-1))),axis=-1)*1000
print 'icvl hier_derot_recur_v2 error ', numpy.mean(mean_err)


# err= numpy.max(numpy.sqrt(numpy.sum((hier_derot_recur_xyz[:,1:21,:] - xyz_true[:,1:21,:])**2,axis=-1)),axis=-1)*1000
err= numpy.max((numpy.sqrt(numpy.sum((hier_derot_recur_xyz- xyz_true)**2,axis=-1)))[:,idx],axis=-1)*1000
# err= numpy.max((numpy.sqrt(numpy.sum((hier_derot_recur_xyz- xyz_true)**2,axis=-1))),axis=-1)*1000

for t in xrange(len(thresholds)):
    loc_bl_t = numpy.where(err <= thresholds[t])
    hier_derot_recur_suc_rate[t] = 100.0 * loc_bl_t[0].shape[0] / err.shape[0]


suc_rate_D = range(5,81,5)
plt.figure()
c5, = plt.plot(suc_rate_D, Sep_Hand_Iter9_suc,linewidth=1, c=color[4],hold=True)
c1, = plt.plot(thresholds,hier_derot_recur_suc_rate,linewidth=1,c=color[0], hold=True)
...
plt.legend([c1,c2,c3,c4,c5,c6], ['DJIF','Melax et al.','HSO','LRF','Sun et al.','Keskin et al.'],loc=4)
...
plt.grid('on')
# plt.show()
plt.savefig('C:/Proj/Proj_Hier_Hand_eccv2016_07042016/Proj_CNN_Hier/data/HDJIF_cmp_prior/eccv2016_fig/hiericvlcmpmax.eps',format='eps', dpi=300)
