...
def test_model_conv3(dataset,setname,dataset_path_prefix, source_name,batch_size,jnt_idx,
    ...
    save_path =   '%sdata/%s/hier_derot_recur_v2/bw_initial/best/'%(dataset_path_prefix,setname)
...
def get_bw_initial_xyz_err(setname,dataset_path_prefix):
    ...
    if setname =='icvl':
        ...
        derot_source_name=File_Name_HDR.icvl_iter0_whlimg_derot
        model_path=File_Name_HDR.icvl_bw_initial_model
        batch_size =133
    else:
        if setname=='nyu':
            source_name=File_Name_HDR.nyu_source_name
            derot_source_name=File_Name_HDR.nyu_iter0_whlimg_derot
            model_path=File_Name_HDR.nyu_bw_initial_patch
            batch_size =4
        else:
            if setname =='msrc':
    ...
    pred_bw_uvd= test_model_conv3(dataset=dataset,setname=setname,dataset_path_prefix=dataset_path_prefix,
    ...
    err_ori_xyz = err_in_ori_xyz(setname,pred_bw_uvd.reshape(pred_bw_uvd.shape[0],6,3),uvd_gr,xyz_true,roixy,rect_d1d2w,depth_dmin_dmax,orig_pad_border,jnt_type=None,jnt_idx=jnt_idx)
    derot_dataset(setname=setname,source_name=source_name,derot_source_name=derot_source_name,pred_uvd=pred_bw_uvd.reshape(pred_bw_uvd.shape[0],6,3))
