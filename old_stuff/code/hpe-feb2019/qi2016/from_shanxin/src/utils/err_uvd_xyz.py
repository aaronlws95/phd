...
def show_ori_multijnt(setname,i,uvd_pred,new_uvd_gr):
    if setname == 'msrc':
            ...
            depth = cv2.imread('%s%s_%s_depth.png' % (dataset_dir, fram_prefix, filename_prefix))
    ...
    if setname == 'nyu':
            ...
            dataset_dir = 'C:\\Proj\\msrc_data\\2014-05-19 personalizations global\\%s\\'%data_set
            filename_prefix = "%06d" % (i)
            depth =  cv2.imread('%s%s_%s.png' % (dataset_dir, fram_prefix, filename_prefix))
            depth = numpy.asarray(depth, dtype='uint16')
    ...
    if setname == 'icvl':
            ...
            depth =  cv2.imread('%s%s%s.png' % (dataset_dir, fram_prefix, filename_prefix))
            depth = numpy.asarray(depth, dtype='uint16')
...
def uvd_to_xyz_error_single_v2(setname,uvd_pred_offset,rot,prev_jnt_uvd_derot,patch_size,jnt_idx ,offset_depth_range,uvd_gr,xyz_true,roixy,rect_d1d2w,depth_dmin_dmax,orig_pad_border):

    predict_uvd =  uvd_pred_offset/10+prev_jnt_uvd_derot
    ...
    if numpy.all(numpy.isnan(xyz_pred)):
        sys.exit('some value of uvd_norm in cnn result is nan')
    return xyz_pred,err
...
def xyz_to_uvd_derot(xyz,setname,rot,jnt_idx,roixy,rect_d1d2w,depth_dmin_dmax,orig_pad_border):
    ...
    if numpy.any(numpy.isnan(uvd_norm)):
        sys.exit('some value of uvd_norm in xyz_to_uvd_derot is nan')
    print 'warning number of prediction below 0 and above 1',numpy.where(uvd_norm[:,0:2]<0)[0].shape[0]+numpy.where(uvd_norm[:,0:2]>1)[0].shape[0]

    uvd_norm[:,0:2][numpy.where(uvd_norm[:,0:2]<0)]=0.0
    uvd_norm[:,0:2][numpy.where(uvd_norm[:,0:2]>1)]=1.0
