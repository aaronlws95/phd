...
def offset_to_abs(off_uvd, pre_uvd,patch_size=44,offset_depth_range=1.0,hand_width=96):
    ...
    if len(off_uvd.shape)<3:
        ...
        if offset_depth_range < 0:
            off_uvd[:,2] = off_uvd[:,2]/abs(offset_depth_range)
        else:
...
def crop_bw_ego_conv_patch(r0,r1,r2,uvd,patch_size):
    ...
    c1=r0.shape[1]
    num_cur_jnt=uvd.shape[-2]
    r0_patch = numpy.empty((num_cur_jnt,batch_size,c1,patch_size,patch_size),dtype='float32')
    r1_patch = numpy.empty((num_cur_jnt, batch_size,c1,patch_size,patch_size),dtype='float32')
    r2_patch = numpy.empty((num_cur_jnt,batch_size,c1,patch_size/2,patch_size/2),dtype='float32')

    for i in xrange(r0.shape[0]):
        for j in xrange(num_cur_jnt):
