...
class EK_HPO_Action_Noun(Model):
        ...
        if load_epoch == 0:
            if self.pretrain:
                if 'state' in self.pretrain:
                    ...
                    cur_dict['module_list.30.conv_30.weight'][:675, :, :, :] = state_dict['module_list.30.conv_30.weight']
                    cur_dict['module_list.30.conv_30.bias'][:675] = state_dict['module_list.30.conv_30.bias']
    ...
    def train_step(self, data_load):
        img, action_gt, noun_gt             = data_load
        batch_size                          = img.shape[0]
        img                                 = img.cuda()
        action_gt                           = action_gt.cuda()
        noun_gt                             = noun_gt.cuda()
        out                                 = self.net(img)[0]
        loss, *hand_losses                  = self.loss(out, action_gt, noun_gt)

        self.optimizer.zero_grad()
        loss.backward()
        self.optimizer.step()

        loss_action, loss_noun = hand_losses
        loss_dict = {
            'loss'          : loss.item(),
    ...
    def detect(self, img):
        with torch.no_grad():
            ...
            pred_conf       = pred_conf.contiguous().view(-1)
            top_idx         = torch.topk(pred_conf, 1)[1]
            pred_action     = pred_hand[64:(64 + self.num_action), :, :, :]
            pred_noun       = pred_hand[(64 + self.num_action):, :, :, :]
            
            uvd_out     = np.zeros((len(pred_conf), 21, 3))
            action_out  = np.zeros(len(pred_conf))
            noun_out    = np.zeros(len(pred_conf))
            
            for idx in range(len(pred_conf)):
                best_pred_uvd   = pred_uvd[:, :, idx].squeeze().cpu().numpy()
                best_pred_uvd   = IMG.scale_points_WH(best_pred_uvd, 
                                                    (1, 1),
                                                    (416, 416))
                best_pred_uvd[..., 2]   *= FPHA.REF_DEPTH
                
                best_pred_action        = pred_action.contiguous().view(self.num_action, -1)[:, idx].squeeze()
                best_pred_noun          = pred_noun.contiguous().view(self.num_noun, -1)[:, idx].squeeze()
                top1_action             = torch.topk(best_pred_action, 1)[1].cpu().numpy()
                top1_noun               = torch.topk(best_pred_noun, 1)[1].cpu().numpy()

                uvd_out[idx] = best_pred_uvd
                action_out[idx] = top1_action
                noun_out[idx] = top1_noun
                
            return uvd_out, action_out, noun_out, top_idx