...
class FPHA_Multireso_Crop_Hand(data.Dataset):
    """ FPHA cropped hand image GT, hand keypoint GT """
    ...
    def __getitem__(self, index):
        ...
        Out:
            img_list        : Multireso cropped hand images 
                              [img0, img0/2, img0/4]
            uvd_gt          : Cropped hand keypoint (21, 3)
            scaling         : Values for reverting hand keypoints to original
                              form [x_min, y_min, crop_w, crop_h]
        """                    
        ...
        # Crop
        img, uvd_gt, x_min, y_min = FPHA.crop_hand(np.asarray(img), uvd_gt)
        img = Image.fromarray(img)
        crop_w = img.size[0]
        crop_h = img.size[1]

        uvd_gt  = IMG.scale_points_WH(uvd_gt, 
                                      (img.size[0], img.size[1]), (1,1))
        ...
        return (img_list, uvd_gt, (x_min, y_min, crop_w, crop_h))
