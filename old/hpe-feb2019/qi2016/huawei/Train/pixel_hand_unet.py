...
lr=0.001
version = 'pixel_ker%d_lr%f'%(num_kern[0],lr)
...
def load_train_test_data():
    # train_x0=numpy.empty((861440,128,160,1),dtype='float32')/2000.0
    # train_mask=numpy.empty((861440,128,160,1),dtype='uint8')

    train_x0=numpy.empty((95744,128,160,1),dtype='float32')/2000.0
    train_mask=numpy.empty((95744,128,160,1),dtype='uint8')

    test_x0=numpy.empty((95744,128,160,1),dtype='float32')/2000.0
    test_mask=numpy.empty((95744,128,160,1),dtype='uint8')


    f = h5py.File('%s/source/test_mask.h5'%save_dir, 'r')
    train_x0[:,4:train_mask.shape[1]-4,:,0] = f['x'][...]
    train_mask[:,4:train_mask.shape[1]-4,:,0] = f['mask'][...]
    f.close()
    f = h5py.File('%s/source/test_mask.h5'%save_dir, 'r')
    test_x0[:,4:test_x0.shape[1]-4,:,0] = f['x'][...]
    test_mask[:,4:test_x0.shape[1]-4,:,0] = f['mask'][...]
    f.close()

    print(train_x0.shape, test_x0.shape)

    return train_x0,train_mask,test_x0,test_mask
