...
def get_top_loc_err(setname,dataset_path_prefix,file_format):


    jnt_idx_all5 = [[3],[7],[11],[15],[19]]
    dataset='test'
    patch_size=40
    ...
    if setname =='icvl':
        ...
        model_path=File_Name_HDR.icvl_top_model
        xyz_jnt_path=File_Name_HDR.icvl_xyz_top_jnt_save_path
        prev_jnt_path=File_Name_HDR.icvl_xyz_mid_jnt_save_path
        batch_size =133
    else:
        if setname=='nyu':
            source_name=File_Name_HDR.nyu_source_name
            derot_source_name=File_Name_HDR.nyu_iter1_whlimg_derot
            model_path=File_Name_HDR.nyu_top_model
            xyz_jnt_path=File_Name_HDR.nyu_xyz_top_jnt_save_path
            prev_jnt_path=File_Name_HDR.nyu_xyz_mid_jnt_save_path
            batch_size =100
        else:
            if setname =='msrc':
                source_name=File_Name_HDR.msrc_source_name
                derot_source_name=File_Name_HDR.msrc_iter1_whlimg_derot
                model_path=File_Name_HDR.msrc_top_model
                xyz_jnt_path=File_Name_HDR.msrc_xyz_top_jnt_save_path
                prev_jnt_path=File_Name_HDR.msrc_xyz_mid_jnt_save_path
                batch_size =100
            else:
