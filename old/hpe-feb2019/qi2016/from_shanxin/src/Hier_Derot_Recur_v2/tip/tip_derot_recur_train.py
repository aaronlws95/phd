...
def train_model(setname, dataset_path_prefix,source_name,prev_jnt_name,batch_size,jnt_idx,patch_size,offset_depth_range,c1,c2,h1_out_factor,h2_out_factor,lamda):
    ...
    model_info='offset_tip%d_patch%d_beta%d'%(jnt_idx[0],patch_size,beta1*10)
    ...
    train_set_x0, train_set_x1,train_set_x2,train_set_y= load_data_multi_tip_uvd_normalized(batch_size,path,prev_jnt_path,jnt_idx=jnt_idx,is_shuffle=True,
    ...
    prev_jnt_path ='%s%s%s.npy'%(direct,dataset,prev_jnt_name)
    test_set_x0, test_set_x1,test_set_x2,test_set_y= load_data_multi_tip_uvd_normalized(batch_size,path,prev_jnt_path,jnt_idx=jnt_idx,is_shuffle=True,
                                                                                        patch_size=patch_size, patch_pad_width=4,offset_depth_range=offset_depth_range,hand_width=96,hand_pad_width=0)
    ...
    done_looping=False
    cost_tmp=9999
    save_path = '%sdata/%s/hier_derot_recur_v2/tip/'%(dataset_path_prefix,setname)
