...
import numpy
from load_data import  load_data_multi, load_data_multi_shanxin
...
def train_model_conv3(model_info,setname, dataset_path_prefix,source_name,batch_size,jnt_idx,c1,c2,c3,h1_out_factor,h2_out_factor,lamda,
                      trainingdata_name, testingdata_name, Training_Subjects_Names, Testing_Subjects_Names ):
    ...
    #     numpy.concatenate([r0, r0[0:num]], axis=0).reshape(r0.shape[0] + num, 1, r0.shape[1], r0.shape[2]),
    #

    # path = '%s%s.h5' % (trainingdata_name, current_subject)
    train_set_x0, train_set_x1, train_set_x2, train_set_y = load_data_multi_shanxin(batch_size=batch_size, trainingdata_name = trainingdata_name,
                                                                            Training_Subjects_Names = Training_Subjects_Names,
                                                                                is_shuffle=True, jnt_idx=jnt_idx)
    ...
    test_set_x0, test_set_x1, test_set_x2, test_set_y = load_data_multi_shanxin(batch_size=batch_size,
                                                                                    trainingdata_name=testingdata_name,
                                                                                    Training_Subjects_Names=Testing_Subjects_Names,
                                                                                    is_shuffle=True, jnt_idx=jnt_idx)
...
if __name__ == '__main__':
    ...
    # training_data =
    Training_Subjects_Names = []
    Training_Subjects_Names.append('Caner')
    Training_Subjects_Names.append('Guillermo')
    Training_Subjects_Names.append('Patrick')
    Training_Subjects_Names.append('Qi')

    Training_Subjects_Names.append('ShanxinV7')
    Training_Subjects_Names.append('seung')
    Training_Subjects_Names.append('vassileios')
    Training_Subjects_Names.append('Xinghao')
    ...
    trainingdata_name = '/media/Data/shanxin/StructuralHand/Mega/data/ego/Ego_norm_hand_uvd_rootmid_ICCVWorkshop_Ego_'
    testingdata_name = '/media/Data/shanxin/StructuralHand/Mega/data/ego/Ego_norm_hand_uvd_rootmid_ICCVWorkshop_Ego_'

    sourcename = 'ICCVWorkshop_Ego_%s'%Testing_Subjects_Names[0]
    model_info = 'ICCVWorkshop_Ego_%s'%Testing_Subjects_Names[0]
    # batch_size=256
    batch_size=128
    ...
    train_model_conv3(model_info=model_info,
                    ...
                    Training_Subjects_Names = Training_Subjects_Names,
                    Testing_Subjects_Names = Testing_Subjects_Names)
