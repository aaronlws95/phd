...
def train_model_conv3(setname, cross_test_set,num_test_sample,
    ...
    # model_name= 'uvd_whl_r012_21jnts_half_64_96_128_1_2_adam_lm300_ep6_000618'
    # set_adam_params('%sdata/%s/whole/inter/param_cost_%s.npy'%(dataset_path_prefix,setname,model_name),model.params)
    ...
    # result_save_path = "%sdata/%s/whole/result_half"%(dataset_path_prefix,setname)
    numpy.save('%s/%s_%s_bb%d_pred_norm_uvd_%s'%(result_save_path,cross_test_set,dataset,bbsize,model_name),pred_y[0:num_test_sample])

    xyz,xyz_gt = error.normuvd2xyz(cross_test_set,path,pred_y[0:num_test_sample])
    scipy.io.savemat('%s/%s_%s_bb%d_xyz_%s'%(result_save_path,cross_test_set,dataset,bbsize,model_name),
                     {'xyz_pred':xyz,'xyz_jnt_gt':xyz_gt})
    # numpy.save('%s/%s_%s_xyz_%s'%(result_save_path,cross_test_set,dataset,model_name),[xyz.xyz_gt])
...
if __name__ == '__main__':
    ...
    # xyz,xyz_jnt_gt = error.normuvd2xyz(setname=cross_test_set,path=path,cur_norm_uvd=pred_y)
    ...
    #
    # for bbsize in xrange(220,281,20):
    #     print 'bbsize=',bbsize
