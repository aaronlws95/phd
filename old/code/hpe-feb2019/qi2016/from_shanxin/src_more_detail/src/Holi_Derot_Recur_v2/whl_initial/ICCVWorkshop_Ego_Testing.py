...
def train_model_conv3(model_info, cross_test_set,num_test_sample,
    ...
    # model_info='uvd_w_r012_21jnts_only_testing'
    print model_info
    ...
    set_adam_params('%s/ego/param_cost_%s.npy'%(dataset_path_prefix,model_name),model.params)
    ...
    result_save_path = "%s/ego/result_%s"%(dataset_path_prefix,source_name)
    # numpy.save('%s_%s_%s_bb%d_pred_norm_uvd_%s'%(result_save_path,trainortest,bbsize,model_name),pred_y[0:num_test_sample])
    ...
    scipy.io.savemat('%s_bb%d_xyz_%s'%(result_save_path,bbsize,model_name),
...
if __name__ == '__main__':
    ...
    Testing_Subjects_Names = []
    Testing_Subjects_Names.append('Xinghao')
    Testing_Subjects_Names.append('vassileios')
    Testing_Subjects_Names.append('ShanxinV7')
    Testing_Subjects_Names.append('sara')
    Testing_Subjects_Names.append('Caner')
    Testing_Subjects_Names.append('Guillermo')
    Testing_Subjects_Names.append('Patrick')
    Testing_Subjects_Names.append('seung')
    Testing_Subjects_Names.append('Qi')

    num_test_sample_all = [33754,  # 'Xinghao'
                           33928,  # 'vassileios'
                           33755,  # 'ShanxinV7'
                           31773,  # 'sara'
                           33224,  # 'Caner'
    ...
    for person in range(0, 9):
        Testing_Subjects_Name = Testing_Subjects_Names[person]
        num_test_sample = num_test_sample_all[person]
        print 'Testing_Subjects_Name', Testing_Subjects_Name
        print 'num_test_sample', num_test_sample

        sourcename = 'ICCVWorkshop_Ego_%s' % Testing_Subjects_Names[person]
        model_info = 'ICCVWorkshop_Ego_%s' % Testing_Subjects_Names[person]

        bbsize = 260
        print 'bbsize=', bbsize
        testingdata_name = '/media/Data/shanxin/StructuralHand/Mega/data/ego/Ego_norm_hand_uvd_rootmid_ICCVWorkshop_Ego_%s.h5'%Testing_Subjects_Name
        model_name='%s_64_96_128_1_2_adam_lm99'%sourcename
