...
import numpy
from load_data import  load_data_multi,load_data_multi_half_train
...
def train_model_conv3(setname, dataset_path_prefix,source_name,batch_size,jnt_idx,c1,c2,c3,h1_out_factor,h2_out_factor,lamda):
    ...
    model_info='uvd_whl_r012_21jnts_8'
    print model_info
    src_path = '%sdata/%s/source/'%(dataset_path_prefix,setname)
    dataset = 'train'
    path = '%smega_%s%s_8.h5'%(src_path,dataset,source_name)
    ...
    dataset = 'test'
    path = '%smega_%s%s_half.h5'%(src_path,dataset,source_name)
    ...
    if interupt == 0:
        ...
        m,v,beta1_t,beta2_t,train_cost,test_cost = set_adam_params('%sdata/%s/whole/param_cost_uvd_whl_r012_21jnts_64_96_128_1_2_adam_lm99_bf.npy'%
