...
class CNN_Model_multi3_conv3(object):
    ...
    def save_adam(self,path,m,v,beta1_t,beta2_t,c00,c01,c02,h1_out_factor,h2_out_factor,lamda,epoch,train_cost,test_cost):
        m_val=[]
        v_val=[]

        params_value = []
        for param_i,m_i,v_i in zip(self.params,m,v):
            m_val.append(m_i.get_value())
            v_val.append(v_i.get_value())

            params_value.append(param_i.get_value())
        np.save("%sparam_cost_%s_%d_%d_%d_%d_%d_adam_lm%d"%(path,self.model_info,c00,c01,c02,h1_out_factor,h2_out_factor,lamda),
                [params_value,m_val,v_val,beta1_t.get_value(),beta2_t.get_value(),self.hyper_params,train_cost,test_cost])

class CNN_Model_multi3_conv3_layer1(object):
    def __init__(self,
                 model_info,
                 X0,
                 X1,
...
class CNN_Model_multi3(object):
    ...
    def save_adam(self,path,m,v,beta1_t,beta2_t,c00,c01,h1_out_factor,h2_out_factor,lamda,train_cost,test_cost):
        m_val=[]
        v_val=[]
        ...
        for param_i,m_i,v_i in zip(self.params,m,v):
            ...
            params_value.append(param_i.get_value())
        np.save("%sparam_cost_%s_%d_%d_%d_%d_adam_lm%d"%(path,self.model_info,c00,c01,h1_out_factor,h2_out_factor,lamda),
                [params_value,m_val,v_val,beta1_t.get_value(),beta2_t.get_value(),self.hyper_params,train_cost,test_cost])
...
class CNN_Model_multi3_conv1(object):
    ...
    def save_adam(self,path,c00,h1_out_factor,h2_out_factor,lamda,epoch,train_cost,test_cost):
        params_value = []
        for param_i in self.params:
            params_value.append(param_i.get_value())
        np.save("%sparam_cost_%s_%d_%d_%d_adam_lm%d_ep%d"%(path,self.model_info,c00,h1_out_factor,h2_out_factor,lamda,epoch),[params_value,self.hyper_params,train_cost,test_cost])
        print "%sparam_cost_%s_%d_%d_%d_adam_lm%d_ep%d"%(path,self.model_info,c00,h1_out_factor,h2_out_factor,lamda,epoch)
