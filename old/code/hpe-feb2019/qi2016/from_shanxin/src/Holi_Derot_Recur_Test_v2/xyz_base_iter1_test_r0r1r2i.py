...
def train_model_multi3(dataset,setname, dataset_path_prefix,source_name,batch_size,jnt_idx,patch_size,c1,c2,h1_out_factor,h2_out_factor,model_save_path):
    ...
    save_path = '%sdata/%s/holi_derot_recur_v2/bw_offset/best/'%(dataset_path_prefix,setname)
...
def get_bw_xyz_err(setname,dataset_path_prefix):
    ...
    if setname =='icvl':
        ...
        model_path=File_Name_HoliDR.icvl_bw_model
        xyz_jnt_path=File_Name_HoliDR.icvl_xyz_bw_jnt_save_path
        bw_initial_patch = File_Name_HoliDR.icvl_iter0_whlimg_derot
        batch_size =133
        ...
        if setname=='nyu':
            ...
            model_path=File_Name_HoliDR.nyu_bw_model
            xyz_jnt_path=File_Name_HoliDR.nyu_xyz_bw_jnt_save_path
            bw_initial_patch = File_Name_HoliDR.nyu_iter0_whlimg_derot
            batch_size =4
            ...
            if setname =='msrc':
                ...
                model_path=File_Name_HoliDR.msrc_bw_model
                xyz_jnt_path=File_Name_HoliDR.msrc_xyz_bw_jnt_save_path
                bw_initial_patch = File_Name_HoliDR.msrc_iter0_whlimg_derot
                batch_size =200
                gr_xyz_path = '%sdata/%s/source/%s_%s_xyz_21joints.mat' % (dataset_path_prefix,setname,dataset,setname)
                gr_roixy_path = '%sdata/%s/source/%s_%s_roixy_21joints.mat' % (dataset_path_prefix,setname,dataset,setname)
            else:
    ...
    pred_uvd_derot_0 = f['pred_uvd_derot'][...]
    rot = f['rotation'][...]
    f.close()
    pred_uvd_derot_0.shape =(pred_uvd_derot_0.shape[0],21,3)
    ...
    pred_bw_uvd_derot_0=pred_uvd_derot_0[:,jnt_idx,:]
    ...
    pred_uvd_derot_0[:,jnt_idx,:]=uvd_offset_norm/10+pred_bw_uvd_derot_0
    """"rot the the norm view to original rotatioin view"""
    for i in xrange(pred_uvd_derot_0.shape[0]):
        M = cv2.getRotationMatrix2D((48,48),rot[i],1)
        for j in xrange(pred_uvd_derot_0.shape[1]):
            pred_uvd_derot_0[i,j,0:2] = (numpy.dot(M,numpy.array([pred_uvd_derot_0[i,j,0]*96,pred_uvd_derot_0[i,j,1]*96,1]))-12)/72
    ...
    xyz,err = err_in_ori_xyz(setname, pred_uvd_derot_0[:,jnt_idx,:],uvd_gr,xyz_true,roixy,rect_d1d2w,depth_dmin_dmax,orig_pad_border,jnt_type=None,jnt_idx=jnt_idx)
    ...
    derot_dataset(setname=setname,source_name=source_name,derot_source_name=derot_source_name,pred_uvd=pred_uvd_derot_0)
