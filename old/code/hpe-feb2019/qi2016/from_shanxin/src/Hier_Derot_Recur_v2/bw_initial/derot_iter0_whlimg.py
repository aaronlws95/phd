...
def derot_dataset(dataset,bw_inital_uvd,setname,source_name):
    ...
    direct = '%sdata/%s/hier_derot_recur_v2/bw_initial/best/'%(constants.Data_Path,setname)
    prev_jnt_path ='%s%s%s.npy'%(direct,dataset,bw_inital_uvd)
    pred_bw_uvd = numpy.load(prev_jnt_path)
    pred_bw_uvd.shape = (pred_bw_uvd.shape[0],6,3)
    ...
    f_derot.create_dataset('pred_bw_uvd', data=pred_bw_uvd)
...
if __name__ == "__main__":
    ...
    derot_dataset(dataset='test',setname='icvl',
                  ...
                  bw_inital_uvd='_uvd_bw_r012_21jnts_64_96_128_1_1_adam_lm9_ep33')
