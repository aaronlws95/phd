...
if __name__ =='__main__':
    ...
    slice_range = 900
    xyz_hybrid =numpy.empty((slice_range,11,3),dtype='float32')
    for i,idx in enumerate(part_jnt_idx_us):
        xyz_hybrid[:,i,:] = scipy.io.loadmat('C:/Proj/Proj_CNN_Hier/data/nyu/nyu_cnn_pso_1000/jnt%d_xyz.mat'%idx)['jnt'][0:slice_range]

    xyz_hso = scipy.io.loadmat('C:/Proj/Proj_CNN_Hier/data/HDJIF_cmp_prior/Result_Prior_Cmp/%s_hso_8252_21jnts'%setname)['xyz'][:,part_jnt_idx_us,:][0:slice_range]


    uvd_fb= numpy.loadtxt('D:\\Project\\3DHandPose\\Data_3DHandPoseDataset\\Exp_Result_Prior_Work\\ICCV15_Feedback\\ICCV15_NYU_Feedback.txt')[0:slice_range]
    uvd_fb.shape=(uvd_fb.shape[0],14,3)
    uvd_hd= numpy.loadtxt('D:\\Project\\3DHandPose\\Data_3DHandPoseDataset\\Exp_Result_Prior_Work\\ICCV15_Feedback\\CVWW15_NYU_Prior-Refinement.txt')[0:slice_range]
    ...
    xyz_true = keypoints['xyz'][:,part_jnt_idx_us,:][0:slice_range]
    print xyz_true.shape
    # keypoints = scipy.io.loadmat('%sdata/%s/source/%s_%s_uvd_21joints_ori.mat' % (dataset_path_prefix,setname,dataset,setname))
    ...
    for image_index in xrange(100,190,3):
        ...
        err = numpy.mean(numpy.sqrt(numpy.sum((xyz_hd[image_index]-xyz_true[image_index])**2,axis=-1)))*1000
        plt.text(h0_off+step*2,v0_off,'%.0f mm'%err,ha='center', va='center',size=20,color='k')

        err = numpy.mean(numpy.sqrt(numpy.sum((xyz_fb[image_index]-xyz_true[image_index])**2,axis=-1)))*1000
        ...
        plt.savefig('C:/Proj/Proj_CNN_Hier/data/HDJIF_cmp_prior/%s_CMP_VIDEO_ECCV2016_sampl/%sCMP%04d.png'%(setname,setname,image_index+1),format='png',dpi=300)
        plt.close('all')
