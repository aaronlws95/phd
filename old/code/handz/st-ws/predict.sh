...
echo CONF=$CONF
echo EPOCH=$EPOCH
srun -p Platform --mpi=pmi2 --gres=gpu:$N_GPU -n1 --ntasks-per-node=$N_GPU --job-name=$EXP_NAME --kill-on-bad-exit=1 -w SH-IDC1-10-5-34-${NODE} python predict.py --conf $CONF --epoch $EPOCH