...
class ConcatData_Bbox_Dataset(torch.utils.data.Dataset):
    def __init__(self, cfg, split):
        ...
        dataset_list = [i for i in cfg['dataset_list'].split(',')]

        for dataset in dataset_list:
            split_set = Path(ROOT)/(cfg['{}_set_{}'.format(split, dataset)])
            with open(split_set, 'r') as f:
                img_paths = f.read().splitlines()

            split_set = Path(ROOT)/(cfg['{}_set_{}'.format(split, dataset)]).replace('img', 'bbox_pad{}'.format(cfg['pad_{}'.format(dataset)]))
            bbox_gt = np.loadtxt(split_set)

            img_root = Path(ROOT)/cfg['img_root_{}'.format(dataset)]

            for i, b in zip(img_paths, bbox_gt):
                self.img_paths.append(img_root/i)
                bbox_norm = b.copy()
                bbox_norm[0] /= int(cfg['img_ref_width_{}'.format(dataset)])
                bbox_norm[1] /= int(cfg['img_ref_height_{}'.format(dataset)])
                bbox_norm[2] /= int(cfg['img_ref_width_{}'.format(dataset)])
                bbox_norm[3] /= int(cfg['img_ref_height_{}'.format(dataset)])
                self.bbox_gt.append(bbox_norm)

        # np.savetxt(Path(ROOT)/'mlcv-exp/data/labels/{}_bbox_norm_{}.txt'.format(cfg['exp'], split), self.bbox_gt)
        ...
        if cfg['aug']:
            ...
            if cfg['zoom']:
                ...
                tfrm.append(ImgBboxZoom(zoom_factor))
