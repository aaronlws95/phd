...
class Multireso_Net(nn.Module):
  def __init__(self, num_filter, channels=3):
...
class Multireso_Net_Batchnorm(nn.Module):
  def __init__(self, num_filter, channels=3):
    super(Multireso_Net_Batchnorm, self).__init__()
    self.conv_block_0 = self._conv_block(num_filter, [5, 4, 3], [4, 2, 2], channels)
    self.conv_block_1 = self._conv_block(num_filter, [5, 3, 3], [2, 2, 2], channels)
    self.conv_block_2 = self._conv_block(num_filter, [5, 5, 3], [2, 1, 1], channels)
    self.dense_block = self._dense_block(6144)
  ...
  def _conv_block(self, num_filter, k, p, channels):
    def conv_and_maxpool(num_filter, k, p, channels):
      conv = nn.Conv2d(channels, num_filter, kernel_size=k)
      batchnorm = nn.BatchNorm2d(num_features=num_filter)
      relu = nn.ReLU()
      maxpool = nn.MaxPool2d(kernel_size=p)
      return nn.Sequential(conv, batchnorm, relu, maxpool)
  ...
  def _dense_block(self, in_features):
    batchnorm_prev = nn.BatchNorm1d(num_features=in_features)
    d0 = nn.Linear(in_features, in_features//2)
    batchnorm0 = nn.BatchNorm1d(num_features=in_features//2)
    relu0 = nn.ReLU()
    d1  = nn.Linear(in_features//2, in_features//4)
    batchnorm1 = nn.BatchNorm1d(num_features=in_features//4)
    relu1 = nn.ReLU()
    d2 = nn.Linear(in_features//4, 63)
    return nn.Sequential(batchnorm_prev, d0, batchnorm0, relu0, d1, batchnorm1, relu1, d2)
...
class Multireso_Net_Batchnorm_6144_3072(nn.Module):
  def __init__(self, num_filter, channels=3):
    super(Multireso_Net_Batchnorm_6144_3072, self).__init__()
    self.conv_block_0 = self._conv_block(num_filter, [5, 4, 3], [4, 2, 2], channels)
    self.conv_block_1 = self._conv_block(num_filter, [5, 3, 3], [2, 2, 2], channels)
    self.conv_block_2 = self._conv_block(num_filter, [5, 5, 3], [2, 1, 1], channels)
    self.dense_block = self._dense_block(6144)
  ...
  def _dense_block(self, in_features):
    batchnorm_prev = nn.BatchNorm1d(num_features=in_features)
    d0 = nn.Linear(in_features, in_features//1)
    batchnorm0 = nn.BatchNorm1d(num_features=in_features//1)
    relu0 = nn.ReLU()
    d1  = nn.Linear(in_features//1, in_features//2)
    batchnorm1 = nn.BatchNorm1d(num_features=in_features//2)
    relu1 = nn.ReLU()
    d2 = nn.Linear(in_features//2, 63)
    return nn.Sequential(batchnorm_prev, d0, batchnorm0, relu0, d1, batchnorm1, relu1, d2)
